...
$(document).ready(function () {
  /** socket untuk berkomunikasi dengan whatsapp Start--> */
  socket.on("message", function (msg) {
    logMsg(null, msg, null);
    if (msg === "QR Code received, scan please!") {
      showQR(true);
    }
    if (msg === "Whatsapp is connected") {
      showQR(false);
    }
  });
  ...
  /** socket untuk berkomunikasi dengan whatsapp <--End */

  /** Script untuk halaman broadcast-mudah */
  if (url.indexOf("broadcast-mudah") > -1) {
    select2Numbers();
    modalOptionKirim();
    clearMessage1();
    sendModeMudah();
  ...
  /** Script untuk halaman broadcast-susah */
  if (url.indexOf("broadcast-susah") > -1) {
    select2Numbers();
    modalOptionKirim();
    clearMessage1();
    sendModeSusah();
  }
  /** Script untuk halaman tools-grabcontact */
  if (url.indexOf("tools-grabcontact") > -1) {
    grabContactGroup();
  }
    ...
    if (obj["status"] === true) {
      ...
      if (dataGroup.length > 0) {
        ...
        for (let i = 0; i < dataGroup.length; i++) {
          ...
          html += "<td>" + c + "</td>";
          html += "</tr>";
          jmlk += parseInt(cs.length);
        }
        html += "</tbody></table>";
        $(".result-grabcontact").html(html);
        $("button#grab-allcontact").html("Grab All");
        $(".jmlgrup").html(dataGroup.length + " group");
        $(".jmlkontak").html(jmlk + " kontak");
        $("table").DataTable({
          responsive: true,
        });
...
function showQR(p = true) {
  if (p == true) {
    ...
    $("div.iq-sidebar").hide();
    $("#msgscan").html("QR Code received, scan please!");
  }
  if (p == false) {
